...
public final class WithDrawSavingsCommand implements Command {
            ...
            for (Account account : user.getSavings()) {
                if (account.getIban().equals(
                    ...
                    for (Account acc : user.getClassics()) {
                        if (acc.getCurrency().equals(
                                commandInput.getCurrency())) {
                            classicFound = true;
                            if (calculateAge(user) != null
                                    && calculateAge(user).getYears() >= 21) {
                                ...
                                if (exchangedAmount <= account.getBalance()) {
                                    ...
                                    Transaction transaction = new WithDrawSavingsTransaction(
                                            commandInput.getTimestamp(), "Savings withdrawal",
                                            account.getIban(), acc.getIban(),
                                            commandInput.getAmount());
                                    user.addTransaction(transaction);
                                    user.addTransaction((transaction));
                                } else {
                                    Transaction transaction = new WithDrawSavingsErrorTransaction(
                    ...
                    if (!classicFound) {
                        Transaction transaction = new WithDrawSavingsErrorTransaction(
                                commandInput.getTimestamp(), "You do not have a classic account.");
                        user.addTransaction(transaction);
    ...
    /**
     * functie pentru calcularea varstei.
     */
