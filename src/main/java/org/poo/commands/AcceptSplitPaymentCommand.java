...
public final class AcceptSplitPaymentCommand implements Command {
    /**
     * functia de executare pentru acceptarea unei plati distribuite aflate in asteptare.
     */
        ...
        for (User user : infoBank.getUsers()) {
            if (user.getEmail().equals(commandInput.getEmail())) {
                ...
                if (!user.getSplitPayments().isEmpty()) {
                                ...
                                for (String iban : user.getSplitPayments().get(0).getAccounts()) {
                                    if (account.getIban().equals(iban)) {
                                        for (SplitPayment splitPayment : user1.getSplitPayments()) {
                                            if (splitPayment.equals(
                                                    user.getSplitPayments().get(0))) {
                                                splitPayment.setAccepts(
                                                        splitPayment.getAccepts() + 1);
                                            }
                                        }
                                    }
                                }
                    ...
                    user.getSplitPayments().get(0).setAccepts(
                            user.getSplitPayments().get(0).getAccepts() + 1);
                    if (user.getSplitPayments().get(0).getAccepts()
                            == user.getSplitPayments().get(0).getAccounts().size()) {
                        ExecuteSplitPayment executeSplitPaymentCommand =
                                new ExecuteSplitPayment();
                        executeSplitPaymentCommand.execute(
                                user.getSplitPayments().get(0), infoBank, objectMapper, output);
                    }
